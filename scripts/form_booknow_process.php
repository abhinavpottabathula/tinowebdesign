...
function process_form($form, $keycode) {
	...
	email_form_submission($form, $keycode);
}
...
function email_form_submission($form, $keycode) {
	if(!defined('PHP_EOL'))
		define('PHP_EOL', '\r\n');
...
function get_email_body($subject, $heading, $fields, $resources) {
	...
	$message .= '<p>Dear ' . $_POST['custom_U2465'] . ',</p>';
	$message .= '<p>Thank you for choosing Tecno! We are pleased to inform you that your reservation for ' . $_POST['custom_U2639'] . ' has been confirmed. We look forward to seeing you then!</p>';
	$message .= '<p>For future reference, your confirmation code is: ' . $_POST['custom_U2740'] . '</p>';
	$message .= '<p>Please confirm the following details.</p>';
	...
	// 	$message .= '<tr><td valign="top" style="background-color: #ffffff;"><b>' . encode_for_form($field_wrapper['properties']['label']) . ':</b></td><td>';
	// 	if ((String)(get_form_field_value($field_wrapper['field'], $field_wrapper['properties'], $resources, true)) != "Cupertino") {
	// 		$message .= get_form_field_value($field_wrapper['field'], $field_wrapper['properties'], $resources, true);
	// 	} else {
	// 		$message .= "" . $_POST['calendar_date'];
	// 	}
	// 	$message .= '</td></tr>';
	// }
	foreach ($sorted_fields as $field_wrapper)
	...
	$message .= '<p>Note: To protect our customers’ identities, we do not ask for payment information online. Payments can be made in-store.</p>';
	$message .= '<p>If you believe that there is an error with the information above, please contact us at (408) 366-7300. If you would like to change or cancel your booking at any time, you can do so <a href="http://tecnoparty.cf/check-order.html">here</a>.</p>';
	$message .= '<p>We hope to see you soon!</p>';
	$message .= '<p>Your friends at Tecno!</p>';
